...
- **Workspace bar:** native Option-hold overlay (`revealModifier = "option"`, `revealHoldMilliseconds = 200.0`, `position = "overlappingMenuBar"`, `notchMode = "moveBelowMenuBar"`, `reserveLayoutSpace = true`). Placement and reservation intentionally mirror OmniWM's own normalized values so the running app never drifts the checked-in file. See `docs/hosts/macos.md`.
